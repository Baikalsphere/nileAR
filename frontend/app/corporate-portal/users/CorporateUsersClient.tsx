...
import {
  ...
  updateCorporateUser,
  deleteCorporateUser,
  getCorporateUserRoleFromStorage,
  type CorporatePortalUser,
  type CorporatePortalUserPayload
} from '@/lib/corporateAuth'

const CORPORATE_SIDEBAR_PAGES = [
  { key: 'dashboard', label: 'Dashboard', icon: 'dashboard', description: 'Overview & analytics' },
  { key: 'invoices', label: 'Invoices', icon: 'receipt_long', description: 'Billing & payments' },
  { key: 'users', label: 'Users', icon: 'group', description: 'Team management' },
  { key: 'employee-stays', label: 'Employee Stays', icon: 'luggage', description: 'Travel tracking' },
  { key: 'book-rooms', label: 'Book Rooms', icon: 'hotel', description: 'Room reservations' },
  { key: 'reports', label: 'Reports', icon: 'bar_chart', description: 'Analytics & insights' },
  { key: 'settings', label: 'Settings', icon: 'settings', description: 'Portal configuration' },
]

export default function CorporateUsersClient() {
  const router = useRouter()
  const [users, setUsers] = useState<CorporatePortalUser[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  const [showModal, setShowModal] = useState(false)
  const [editingUser, setEditingUser] = useState<CorporatePortalUser | null>(null)
  ...
  useEffect(() => {
    ...
    const meta = getCorporateUserRoleFromStorage()
    setIsAdmin(meta.role === 'admin' && !meta.isSubUser)
    loadUsers()
  }, [router])
  ...
  const loadUsers = async () => {
    try {
      setLoading(true)
      const data = await fetchCorporateUsers()
      setUsers(data.users)
    } catch (err: any) {
  ...
  const openEditModal = (user: CorporatePortalUser) => {
    setEditingUser(user)
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingUser) {
        await updateCorporateUser(editingUser.id, {
  ...
  const handleToggleActive = async (user: CorporatePortalUser) => {
    try {
      await updateCorporateUser(user.id, { isActive: !user.is_active })
      await loadUsers()
    } catch (err: any) {
  ...
  const handleDelete = async (user: CorporatePortalUser) => {
    if (!confirm(`Delete user "${user.full_name}"? This cannot be undone.`)) return
    try {
      await deleteCorporateUser(user.id)
      await loadUsers()
    } catch (err: any) {
  ...
  return (
    <div className="flex h-screen bg-[#f8fafb] dark:bg-[#0f1623]">
      <Sidebar title="Corporate" logoIcon="business" />
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (formPages.length === CORPORATE_SIDEBAR_PAGES.length) {
                          setFormPages([])
                        } else {
                          setFormPages(CORPORATE_SIDEBAR_PAGES.map(p => p.key))
                        }
                      }}
                      className="text-xs font-medium text-primary hover:text-primary/80 transition-colors"
                    >
                      {formPages.length === CORPORATE_SIDEBAR_PAGES.length ? 'Deselect All' : 'Select All'}
                    </button>
                  </div>
                  <div className="space-y-2">
                    {CORPORATE_SIDEBAR_PAGES.map(page => {
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-[#e7ecf3] dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20 rounded-b-2xl">
              <div className="text-xs text-slate-400">
                {formRole === 'admin' ? (
                  ...
                  <span>{formPages.length} of {CORPORATE_SIDEBAR_PAGES.length} pages selected</span>
                )}
              </div>
              <div className="flex items-center gap-3">
                <button
