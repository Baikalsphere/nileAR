...
export const config = {
  ...
  smtpEnabled: Boolean(env.SMTP_HOST && env.SMTP_USER && env.SMTP_PASS && env.SMTP_FROM),
  resendEnabled: Boolean(env.RESEND_API_KEY && env.SMTP_FROM),
  mailEnabled: env.MAIL_PROVIDER === "resend"
    ? Boolean(env.RESEND_API_KEY && env.SMTP_FROM)
